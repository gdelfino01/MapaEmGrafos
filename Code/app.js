...
const GrafoApp = {
  ...
  init() {
    ...
    document.getElementById("geojsonInput").addEventListener("change", event => {
      ...
      if (file) {
        ...
        reader.onload = e => this.loadGeoJSON(JSON.parse(e.target.result));
  ...
  loadGeoJSON(data) {
    ...
    data.features.forEach(f => {
      ...
      for (let i = 0; i < coords.length - 1; i++) {
        const key1 = coords[i].join(","), key2 = coords[i+1].join(",");
  ...
  drawGraph(path = []) {
    ...
    if (this.selected[0]) {
      ...
      this.ctx.beginPath(); this.ctx.arc(x, y, 6, 0, 2 * Math.PI); this.ctx.fill();
    }
    if (this.selected[1]) {
    ...
    path.forEach((n, i) => {
      ...
      this.ctx.beginPath(); this.ctx.arc(x, y, 5, 0, 2 * Math.PI); this.ctx.fill();
    });
  },
  ...
  handleClick(e) {
    ...
    const rua1 = this.getRua(clicked);
    if (this.selected.length === 1) {
      this.distDiv.textContent = `Ponto inicial: ${rua1}`;
      this.drawGraph();
      ...
      if (confirm(`Deseja encontrar o menor caminho entre ${rua1} e ${rua2}?`)) {
        ...
        const ruas = result.ruas;
        this.logDiv.textContent = `Execuções: ${this.execCount}`;
        this.distDiv.textContent = `Distância total: ${result.total.toFixed(2)} metros`;
